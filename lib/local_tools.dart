...
class GetCurrentWeatherTool extends McpLocalTool {
  ...
  @override
  String get description => 'Get current weather conditions including temperature, wind, humidity, and weather description.';
  ...
  Map<String, dynamic> get inputSchema => {
        ...
        'properties': {
          ...
          'latitude': {'type': 'number', 'description': 'Latitude (optional, overrides location)'},
          'longitude': {'type': 'number', 'description': 'Longitude (optional, overrides location)'},
        },
        'required': [],
      };
    ...
    try {
      ...
      if (lat == null || lng == null) {
        return const MCPToolResult(
          content: [MCPContent(type: 'text', text: 'Error: Could not resolve coordinates.')],
          isError: true,
        );
      }
...
class GetHourlyForecastTool extends McpLocalTool {
  ...
  @override
  String get description => 'Get hourly weather forecast for the next 24-168 hours. Includes temperature, precipitation, wind speed, and weather code.';
    ...
    try {
      ...
      if (hourly != null) {
        ...
        for (int i = 0; i < times.length; i++) {
          ...
          buffer.writeln('- **${times[i].toString().replaceFirst('T', ' ')}**: Temp: ${temps[i]}°C, Humidity: ${humidities[i]}%, ${_wmoCodeToDesc(codeVal)}');
        }
      }
...
class GetDailyForecastTool extends McpLocalTool {
  ...
  @override
  String get description => 'Get daily weather forecast for the next 1-16 days. Includes high/low temperatures, precipitation sum, sunrise/sunset.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    try {
      ...
      if (daily != null) {
        ...
        for (int i = 0; i < times.length; i++) {
          final codeVal = codes[i] is num ? (codes[i] as num).toInt() : 0;
          buffer.writeln('- **${times[i]}**: Min: ${minTemps[i]}°C, Max: ${maxTemps[i]}°C, Rain: ${probs[i]}%, ${_wmoCodeToDesc(codeVal)}');
        }
      }
...
class GeocodeWeatherCityTool extends McpLocalTool {
  ...
  @override
  String get description => 'Look up the coordinates (latitude, longitude) for a city name. Useful for finding coordinates to pass to other weather tools.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    try {
      ...
      if (city.isEmpty) {
        return const MCPToolResult(
          content: [MCPContent(type: 'text', text: 'Error: City name is required.')],
          isError: true,
        );
      }

      final geoUrl = '$_geocodeUrl/search?name=${Uri.encodeComponent(city)}&count=5&language=en&format=json';
      final geoResp = await http.get(Uri.parse(geoUrl)).timeout(const Duration(seconds: 15));
      if (geoResp.statusCode != 200) {
        return MCPToolResult(
      ...
      if (results != null && results.isNotEmpty) {
        for (final item in results) {
          final m = item as Map;
          buffer.writeln('- **${m['name']}**, ${m['country'] ?? ''} (${m['admin1'] ?? ''})');
...
class SshCredentials {
  ...
  static SshCredentials? fromArgsAndConfig(Map<String, dynamic> arguments, Map<String, dynamic>? globalSsh) {
    ...
    final password = (arguments['password'] as String?)?.isNotEmpty == true 
        ...
        : (globalSsh?['password'] as String? ?? globalSsh?['password'] as String?);
    ...
    final privateKey = (arguments['private_key'] as String?)?.isNotEmpty == true 
        ...
        : ((arguments['privateKey'] as String?)?.isNotEmpty == true 
            ...
            : (globalSsh?['privateKey'] as String? ?? globalSsh?['private_key'] as String?));
...
abstract class SshBaseLocalTool extends McpLocalTool {
  ...
  MCPToolResult _noCredsResult() {
    return const MCPToolResult(
      content: [MCPContent(type: 'text', text: 'Error: SSH host or username is not configured. Configure Connection Override settings.')],
      isError: true,
    );
  }
}
...
class SshListDirectoryTool extends SshBaseLocalTool {
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      ...
      final resultText = code == 0 ? stdout : 'Error listing directory: $stderr';
      return MCPToolResult(
...
class SshReadFileTool extends SshBaseLocalTool {
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      client = await creds.connect();
      // Read remote file using standard cat or head
...
class SshDownloadFileTool extends SshBaseLocalTool {
  ...
  @override
  String get description => 'Download a remote file. Returns base-64 encoded content and MIME type.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      ...
      if (code != 0) {
        return MCPToolResult(
          content: [MCPContent(type: 'text', text: 'Error downloading file: $stderr')],
          isError: true,
        );
      }
      ...
      return MCPToolResult(
        content: [
          MCPContent(
            type: 'image', // base64 representation can be rendered in UI if matched or as image
            data: base64String,
            mimeType: lookupMimeType(fileName) ?? 'application/octet-stream',
            text: 'File $fileName downloaded successfully (${bytes.length} bytes).',
          )
        ],
        isError: false,
      );
...
class SshUploadFileTool extends SshBaseLocalTool {
  ...
  @override
  String get description => 'Upload a file to the remote server. Provide path and content.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      ...
      return MCPToolResult(
        content: [MCPContent(type: 'text', text: 'File uploaded successfully to $path (${bytesToUpload.length} bytes).')],
        isError: false,
      );
    } catch (e) {
      return MCPToolResult(
        content: [MCPContent(type: 'text', text: 'SSH connection or SFTP upload failed: $e')],
        isError: true,
      );
    } finally {
      client?.close();
...
class SshExecuteCommandTool extends SshBaseLocalTool {
  ...
  @override
  String get description => 'Run an ad-hoc shell command on the remote server and return stdout/stderr.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      ...
      return MCPToolResult(
        content: [MCPContent(type: 'text', text: 'SSH command execution failed: $e')],
        isError: true,
      );
    } finally {
      client?.close();
...
class SshMakeDirectoryTool extends SshBaseLocalTool {
  ...
  @override
  String get description => 'Create a directory (and all missing parent directories) on the remote server.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      ...
      final resultText = code == 0 ? 'Directory created successfully.' : 'Error creating directory: $stderr';
      return MCPToolResult(
        content: [MCPContent(type: 'text', text: resultText)],
        isError: code != 0,
      );
...
class SshRemoveDirectoryTool extends SshBaseLocalTool {
  ...
  @override
  String get description => 'Remove an EMPTY directory on the remote server. Fails if the directory is not empty.';
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    ...
    try {
      ...
      final resultText = code == 0 ? 'Directory removed successfully.' : 'Error removing directory: $stderr';
      return MCPToolResult(
        content: [MCPContent(type: 'text', text: resultText)],
        isError: code != 0,
      );
...
class CreateChartPngTool extends McpLocalTool {
  ...
  @override
  String get description => 'Generate a PNG chart (line, bar, pie, area, scatter) from x-axis labels and numeric data series. Returns a base64 encoded PNG image.';
  ...
  Map<String, dynamic> get inputSchema => {
        ...
        'properties': {
          ...
          'title': {
            'type': 'string',
            'description': 'Chart title shown at top.'
          },
          'chartTitle': {
            'type': 'string',
          ...
          'data': {
            'type': 'array',
            'description': 'Y-axis numeric values matching labels (for single series).',
            'items': {'type': 'number'}
          },
          'series': {
            'type': 'array',
  ...
  Future<MCPToolResult> execute(Map<String, dynamic> arguments) async {
    try {
      final chartType = ((arguments['chart_type'] ?? arguments['chartType']) as String? ?? 'line')
          .trim()
          .toLowerCase();
      final title = ((arguments['title'] ?? arguments['chartTitle'] ?? arguments['chart_title']) as String?)?.trim();
      ...
      final yAxisRotate = (arguments['yAxisRotate'] as num?)?.toDouble() ?? -90.0;

      final width = ((arguments['width'] as num?)?.toInt() ?? 800).clamp(400, 2400);
      final height = ((arguments['height'] as num?)?.toInt() ?? 500).clamp(300, 1600);

      final xAxisRaw = (arguments['labels'] ?? arguments['xAxis']) as List<dynamic>?;
      ...
      final parsedSeries = _extractSeries(arguments, rawSeries: seriesRaw, lineColors: lineColors);
      ...
      if (needsXAxis && xAxis.isEmpty) {
        // Fallback xAxis labels if not provided
        final maxLen = parsedSeries.map((s) => s.data.length).fold<int>(0, math.max);
  ...
  List<_ChartSeriesData> _extractSeries(Map<String, dynamic> args, {required List<dynamic>? rawSeries, List<Color> lineColors = const []}) {
    ...
    if (dataRaw != null && dataRaw.isNotEmpty) {
      final sName = ((args['title'] ?? args['chartTitle']) as String? ?? 'Data').trim();
      series.add(_parseSingleSeries(sName, dataRaw, lineColors: lineColors, startIndex: series.length));
    }
  ...
  _ChartSeriesData _parseSingleSeries(String name, List<dynamic> dataRaw, {List<Color> lineColors = const [], int startIndex = 0}) {
    ...
    final color = lineColors.isNotEmpty ? lineColors[startIndex % lineColors.length] : defaults[startIndex % defaults.length];
  ...
  Future<Uint8List> _drawChart({
    ...
    for (var i = 0; i <= 5; i++) {
      ...
      _drawText(canvas, yValue.toStringAsFixed(1), Offset(10, y - 8), const TextStyle(fontSize: 12, color: Color(0xFF374151)));
    }
    ...
    if (chartTitle != null && chartTitle.isNotEmpty) {
      _drawText(
        ...
        chartTitle,
        const Offset(16, 14),
        const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF111827)),
        maxWidth: width - 32,
      );
    }
  ...
  Future<Uint8List> _drawPieChart({
    ...
    if (total == 0) {
      _drawText(canvas, 'All values are zero', Offset(width / 2 - 80, height / 2), const TextStyle(fontSize: 14, color: Color(0xFF6B7280)));
    } else {
      const palette = [
        Color(0xFF1E88E5),
        Color(0xFFE53935),
      ...
      for (var i = 0; i < data.length && i < labels.length; i++) {
        final color = series.length == 1 ? palette[i % palette.length] : series[i % series.length].color;
        canvas.drawRect(Rect.fromLTWH(legendX, ly, 12, 12), Paint()..color = color);
        _drawText(
          canvas,
  ...
  void _renderScatter(ui.Canvas canvas, List<_ChartSeriesData> series, double Function(int) mapX, double Function(double) mapY) {
    for (final s in series) {
  ...
  void _drawXLabel(ui.Canvas canvas, String label, double x, double y, double rotateAngle) {
    if (rotateAngle.abs() < 1) {
      _drawText(canvas, label, Offset(x - 20, y), const TextStyle(fontSize: 10, color: Color(0xFF374151)), maxWidth: 60);
    } else {
      final rad = rotateAngle * math.pi / 180.0;
      canvas.save();
      canvas.translate(x, y + 6);
      canvas.rotate(rad);
      _drawText(canvas, label, const Offset(-25, -6), const TextStyle(fontSize: 10, color: Color(0xFF374151)), maxWidth: 60);
      canvas.restore();
    }
  }

  void _drawLegend(ui.Canvas canvas, List<_ChartSeriesData> series, {required int width}) {
    var x = 16.0;
    const y = 42.0;
    ...
    for (final item in series) {
      ...
      _drawText(canvas, item.name, Offset(x + 16, y - 2), const TextStyle(fontSize: 11, color: Color(0xFF1F2937)), maxWidth: 140);
      x += 160;
      if (x > width - 160) x = 16;
    }
  }

  void _drawText(ui.Canvas canvas, String text, Offset offset, TextStyle style, {double? maxWidth, TextAlign align = TextAlign.left}) {
